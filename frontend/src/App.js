...
import { Send, Calendar, Bell, Sparkles, AlertCircle, CheckCircle2, Clock } from "lucide-react";
...
function App() {
  ...
  const [userEmail, setUserEmail] = useState(localStorage.getItem("user_email") || "yashikasahu0109@gmail.com");
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await axios.post("/chat", {
        message,
        email: userEmail // Pass the user email to the backend
      });
      const data = res.data;
      ...
      setChat((prev) => [
        ...prev,
        { role: "ai", content: "❌ Error: Could not reach the planner. Is the backend running?", type: "error" }
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="App">
      <header style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 15, display: "flex", justifyContent: "center", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: "0.8rem", color: "var(--text-dim)" }}>Confirmation Email:</span>
          <input
            type="email"
            value={userEmail}
